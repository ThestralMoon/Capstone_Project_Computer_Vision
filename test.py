...
def view_records(file_path, class_labels, stride = 1, verbose = 1):
    ...
    for im_ind in range(num_records):
        ...
        fname = parsed_example['image/name'].numpy()
...
#     classes_dict = {"person": 1, "box": 2, "car": 3}
#     #Make the following using voc_to_tfr.py
#     data_path = r"train.tfrecord"
#
#     verbose = 1
#     stride = 1
#     view_records(data_path, classes_dict, stride = stride, verbose = verbose)

for i in tqdm(range(config.NUM_LOSS_FUNCS)):
    print(i)
